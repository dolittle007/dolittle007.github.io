...
Rosenblatt introduced a simple rule by introducing the concept of _weights_.
...
determined by whether the weighted sum of each input, **∑wjxj**, _is less than
or greater than some threshold value_. Therefore, _the main idea of a perceptron
algorithm is to learn the values of the weights **w** that are then multiplied
with the input features in order to make a decision whether a neuron fires or
not_. We can write this in a mathematical expression as depicted below:
...
transformed the weighted sum formulation into a dot product of two vectors, _w
(weights)_ and _x (inputs)_, where w⋅x ≡ ∑wjxj. Then, we can move the threshold
to the other side of the inequality and to replace it by a new variable, called
_bias_ b, where b ≡ −threshold. Now, with those modification, our perceptron
...
1.  At the moment, one of the most efficient ways to train a multi-layer neural
    ...
    _differentiable activation function_. However, the Heaviside step function is
...
_The vanishing gradient_. As you can see from the preceding figure, when z, the
...
[lecture](http://cs231n.github.io/neural-networks-1/) says by default, _use the
ReLU non-linearity._
...
Now, you may ask, _“isn’t that a linear function? why do we call ReLu as a
nonlinear function?”_

First, let us first understand what a linear function is.
[Wikipedia](https://en.wikipedia.org/wiki/Linear_function) says that:

> In linear algebra, a linear function is a map _f_ between two [vector
...
Given the definition above, we can see that _max(0, x)_ is a piece-wise linear
...
**_So how do a neural net generates a predictions?_**
...
So the objective is to find some combination of **w’s** and **b** that could
make our cost **J**, as small as possible. To do this, we’ll rely on two
...
is, _how do we know which direction to step?_ _Should we increase or decrease the
value of our parameters?_ We can do a random search, but it will take quite a
...
where **α** is a step size or learning rate and we’ll multiply this with the
...
Here is where we need the **α**, which is a hyperparameter that basically
...
To find the rate of change of **y_hat** w.r.t **z2**, we need to differentiate our
sigmoid activation function with respect to **z**. Assuming we use a sigmoid activation function, we can then compute it as:
