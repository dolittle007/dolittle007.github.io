...
The **pairtools split** command is used to split the final **.pairsam** into two files: **.sam** (or **.bam**) and **.pairs** (**.pairsam** has two extra columns containing the alignments from which the Hi-C pair was extracted, these two columns are not included in **.pairs** files)
...
I also provide a all-in-one bash script (**sam2pairs.sh**).
